...
SPRINT_HISTORY_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sprint_history.json")
...
async def save_sprint_history(project_id: str, request: SprintSaveRequest):
    try:
        data = {}
        if os.path.exists(SPRINT_HISTORY_PATH):
            with open(SPRINT_HISTORY_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
...
TEAM_PROFILES_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "team_profiles.json")
...
async def remove_team_member(username: str):
    try:
        ...
        original_len = len(data["team"])
        data["team"] = [m for m in data["team"] if m["username"] != username]
        ...
        return {"status": "success", "removed": username, "total": len(data["team"])}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
REVIEWS_HISTORY_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tech_reviews.json")

@app.get("/api/reviews/{project_id}")
async def get_reviews(project_id: str):
    if not os.path.exists(REVIEWS_HISTORY_PATH):
        return {"reviews": []}
    with open(REVIEWS_HISTORY_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
...
async def execute_manual_review(project_id: str, mr_iid: int):
    ...
    history = {}
    if os.path.exists(REVIEWS_HISTORY_PATH):
        with open(REVIEWS_HISTORY_PATH, "r", encoding="utf-8") as f:
            history = json.load(f)

    proj_reviews = history.get(project_id, [])
    new_review = {
        "mr_iid": mr_iid,
        "mr_title": mr_data.get("title"),
    ...
    history[project_id] = proj_reviews

    with open(REVIEWS_HISTORY_PATH, "w", encoding="utf-8") as f:
